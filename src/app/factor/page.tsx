...
export default function Factor(){
    ...
    return(
        <div className="container m-auto text-meshki">
           <p className="text-4xl font-bold font-mono my-9">Shopping Cart</p>
           ...
           {cart.map(item=>(
            <div key={item.id} className="flex my-3">
                ...
                <div className="mt-4">
                <p className="text-basis">{item.title}</p>
                <p className="font-bold text-xl">{item.price}$</p>
                <button onClick={()=>updateQuantity(item.id,item.quantity + 1)} className="py-1 px-3 bg-sabz text-kerem border-2 border-bezh m-2 text-xl">+</button>
                <strong>{item.quantity}</strong>
                <button onClick={()=>updateQuantity(item.id,item.quantity - 1)} className="py-1 px-4 bg-sabz text-kerem border-2 border-bezh m-2 text-xl">-</button>
                <button onClick={()=>removeFromCart(item.id)} className="m-2 mt-4"><AiOutlineDelete className="size-5" /></button>
                </div>
            </div>
           ))}
           
           <button onClick={clearCart} className="text-xl rounded-xl ml-5 my-8 bg-meshki border-bezh border-2 p-3 text-kerem">clear factor</button>
           </div>
           <div className=" justify-center basis-1/2 text-meshki">
            <form  className=" border-sabz border-2 w-3/4">
            <p className="text-center font-mono text-3xl mt-3 ">Delivery Information</p>
                <div className="m-4">
                <label className="text-xl  my-3 text-meshki" >FirstName</label>
               <input type="text" placeholder="Paria " className="w-full py-2 px-1 border-2 border-sabz outline-none"/>
               </div>
               <div   className="m-4">
                <label className="text-xl  my-3 text-meshki">LastName</label>
               <input  type="text" placeholder="Abedi "  className="w-full py-2 px-1 border-2 border-sabz outline-none"/></div>
               <div className="m-4">
                <label className="text-xl  my-3 text-meshki">Mobile</label>
               <input type="number" placeholder="+98 9356943000"  className="w-full py-2 px-1 border-2 border-sabz outline-none"/></div>
               <div className="m-4">
                <label className="text-xl  my-3 text-meshki">Country</label>
               <input type="text" placeholder="Iran"  className="w-full py-2 px-1 border-2 border-sabz outline-none"/></div>
               <div className="m-4">
                <label className="text-xl  my-3 text-meshki">City</label>
               <input type="text" placeholder="Tehran"  className="w-full py-2 px-1 border-2 border-sabz outline-none"/></div>
               <div className="m-4">
                <label className="text-xl  my-3 text-meshki">Address</label>
               <input type="text"  className="w-full py-2 px-1 border-2 border-sabz outline-none"/></div>
               <div className="m-4">
                <label className="text-xl  my-3 text-meshki">PostCode</label>
               ...
               <div className="border-t-2 border-sabz w-5/6 m-auto py-4">
                <p className="text-3xl font-bold"><strong className="text-xl text-meshki">Total To Pay:</strong> {totalPrice} $</p>
                <button className="w-full bg-meshki border-bezh text-bezh py-3 border-2 my-3">Buy Now</button>
                </div>
            </form>
           </div>
           </div>
